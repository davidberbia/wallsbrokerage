  ...
  return (
    <div className="mx-auto max-w-6xl space-y-7 pb-10">
      ...
      <section className="panel p-4 sm:p-7">
        ...
        <div className="mt-8 grid grid-cols-1 gap-3 border-t pt-5 sm:flex sm:justify-end">
          {step > 1 && <Button className="min-h-11 w-full sm:w-auto" type="button" variant="outline" onClick={() => setStep((current) => current - 1)}><ChevronLeft /> Précédent</Button>}
          {step < 5 ? <Button className="min-h-11 w-full sm:w-auto" type="button" onClick={next}>Suivant <ChevronRight /></Button> : <Button className="min-h-11 w-full sm:w-auto" type="button" onClick={save} disabled={saving}><Check /> {saving ? "Validation…" : "Valider"}</Button>}
        </div>
      </section>
    </div>
  );
...
function StepIndicator({ step }: { step: number }) {
  return <ol className="grid grid-cols-5 gap-1.5 sm:gap-2" aria-label="Progression du profil">{STEP_LABELS.map((label, index) => <li key={label} className="min-w-0"><div className={`h-1.5 rounded-full ${index < step ? "bg-accent" : "bg-muted"}`} /><span className="mt-2 hidden truncate text-xs text-muted-foreground md:block">{index + 1}. {label}</span></li>)}</ol>;
}
...
function StrategyStep({ criteria, toggleAsset, updateCriteria }: { criteria: CriteriaMap; toggleAsset: (asset: string) => void; updateCriteria: (asset: string, patch: Partial<CriteriaChoice>) => void }) {
  ...
  return <TooltipProvider><div><h2 className="text-xl">Vos classes d’actifs</h2><p className="mt-1 text-sm text-muted-foreground">Cochez vos actifs, puis au moins une stratégie pour chacun.</p><div className="mt-5 grid gap-x-8 gap-y-2 rounded-md border p-4 sm:grid-cols-2">{assetClasses.map((asset) => <ChoiceBox key={asset} label={asset} checked={Boolean(criteria[asset])} onChange={() => toggleAsset(asset)} />)}</div>{selected.length > 0 && <><div className="mt-6 grid gap-3 md:hidden">{selected.map((asset) => <div key={asset} className="rounded-md border p-4"><p className="font-medium">{asset}</p><div className="mt-3 grid gap-2">{STRATEGIES.map((strategy) => <label key={strategy} onClick={() => toggleStrategy(asset, strategy)} className="flex min-h-11 cursor-pointer items-center gap-3 rounded-md border px-3"><Checkbox checked={Boolean(criteria[asset]?.strategies.includes(strategy))} className="pointer-events-none" aria-label={`${asset}, ${strategy}`} /><span className="text-sm">{strategy}</span><Tooltip><TooltipTrigger type="button" onClick={(event) => event.stopPropagation()} className="ml-auto"><Info className="size-4" /></TooltipTrigger><TooltipContent className="max-w-72"><p>{STRATEGY_DEFINITIONS[strategy]}</p></TooltipContent></Tooltip></label>)}</div></div>)}</div><div className="mt-8 hidden overflow-x-auto border-r border-transparent pr-2 md:block"><table className="w-full min-w-[600px] border-collapse text-sm"><thead><tr><th className="border-b p-3 text-left">Classe d’actif</th>{STRATEGIES.map((strategy) => <th key={strategy} className="border-b p-3 text-center"><Tooltip><TooltipTrigger className="inline-flex items-center gap-1 font-semibold">{strategy}<Info className="size-3.5" /></TooltipTrigger><TooltipContent className="max-w-72"><p>{STRATEGY_DEFINITIONS[strategy]}</p></TooltipContent></Tooltip></th>)}</tr></thead><tbody>{selected.map((asset) => <tr key={asset} className="border-b last:border-0"><th className="p-3 text-left font-medium">{asset}</th>{STRATEGIES.map((strategy) => <td key={strategy} className="p-3 text-center"><Checkbox checked={Boolean(criteria[asset]?.strategies.includes(strategy))} onCheckedChange={() => toggleStrategy(asset, strategy)} aria-label={`${asset}, ${strategy}`} /></td>)}</tr>)}</tbody></table></div></>}</div></TooltipProvider>;
}
...
function AmountStep({ criteria, updateCriteria }: { criteria: CriteriaMap; updateCriteria: (asset: string, patch: Partial<CriteriaChoice>) => void }) {
  ...
  return <div><h2 className="text-xl">Montant par opportunité d’investissement</h2><p className="mt-1 text-sm text-muted-foreground">Sélectionnez au moins une tranche pour chaque classe d’actif.</p><div className="mt-6 grid gap-3 md:hidden">{Object.keys(criteria).map((asset) => <div key={asset} className="rounded-md border p-4"><p className="font-medium">{asset}</p><div className="mt-3 grid gap-2">{amountBands.map((band) => <label key={band} onClick={() => toggle(asset, band)} className="flex min-h-11 cursor-pointer items-center gap-3 rounded-md border px-3"><Checkbox checked={Boolean(criteria[asset]?.amountBands.includes(band))} className="pointer-events-none" aria-label={`${asset}, ${band}`} /><span className="text-sm">{band}</span></label>)}</div></div>)}</div><div className="mt-6 hidden overflow-x-auto border-r border-transparent pr-2 md:block"><table className="w-full min-w-[600px] border-collapse text-sm"><thead><tr><th className="border-b p-3 text-left">Classe d’actif</th>{amountBands.map((band) => <th key={band} className="border-b p-3 text-center">{band}</th>)}</tr></thead><tbody>{Object.keys(criteria).map((asset) => <tr key={asset} className="border-b last:border-0"><th className="p-3 text-left font-medium">{asset}</th>{amountBands.map((band) => <td key={band} className="p-3 text-center"><Checkbox checked={Boolean(criteria[asset]?.amountBands.includes(band))} onCheckedChange={() => toggle(asset, band)} aria-label={`${asset}, ${band}`} /></td>)}</tr>)}</tbody></table></div></div>;
}
