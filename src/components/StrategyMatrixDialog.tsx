...
export function StrategyMatrixDialog({
  ...
  const toggleAsset = (asset: string) => {
    ...
    if (on) delete nextBands[asset];
    else nextBands[asset] = nextBands[asset] ?? [];
    onChange({
      ...value,
      assetClasses: on
  ...
  const toggleBand = (asset: string, band: string) => {
    ...
    onChange({
      ...value,
      assetClasses: value.assetClasses.includes(asset)
        ? value.assetClasses
        : [...value.assetClasses, asset],
  ...
  const toggleIn = (key: "strategies" | "regions") => (opt: string) =>
    onChange({
      ...value,
      [key]: value[key].includes(opt)
        ? value[key].filter((v) => v !== opt)
        : [...value[key], opt],
    });

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[92vh] w-[calc(100%-1.5rem)] max-w-3xl flex-col overflow-hidden">
        <DialogHeader>
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-1 pb-2">
          <div>
            <p className="eyebrow mb-2">Classes d'actifs & tranches d'investissement</p>
            <div className="overflow-x-auto rounded-lg border border-border">
              <table className="w-full border-collapse text-sm">
                <thead className="bg-muted/60">
                  <tr>
                    <th className="sticky left-0 z-10 bg-muted/60 px-2 py-1.5 text-left text-xs font-semibold">
                      Classe d'actif
                    </th>
          ...
          <div>
            <p className="eyebrow mb-2">Stratégies</p>
            <Chips
              options={STRATEGIES}
              value={value.strategies}
              onToggle={toggleIn("strategies")}
            />
          </div>

          <div>
            <p className="eyebrow mb-2">Régions ciblées</p>
            <Chips options={REGIONS} value={value.regions} onToggle={toggleIn("regions")} />
          </div>
        </div>
